        ...
        <div className="flex flex-col lg:flex-row lg:items-stretch">
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-1 p-4 px-5">
            ...
            <div className="flex items-center gap-2 text-sm leading-[var(--dp-lh-sm)] text-[var(--dp-body)]">
              ...
              <span className="min-w-0 overflow-hidden text-ellipsis">{product.category}</span>
            </div>

            {/* Stock */}
            <div className="text-sm leading-[var(--dp-lh-sm)] font-normal text-[var(--dp-body)]">
              {t('inStock')} : <span className="tabular-nums">{product.stock}</span>
            </div>
            ...
            <div className="flex flex-wrap gap-2 mt-1">
              {visibleVariants.map((variant) => {
                ...
                return (
                  <span
                    key={variant.label}
                    title={isLowStock ? `${variant.label} · ${t('lowStock')} (≤ ${LOW_STOCK_THRESHOLD})` : variant.label}
