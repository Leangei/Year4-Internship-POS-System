...
export default function DashboardKpiGrid({ cards }: DashboardKpiGridProps) {
  ...
  const revenueSubtitle = cards[0]?.subtitle ?? '+15% ពីខែមុន'
    ...
    <>
      ...
      <div className="rounded-[20px] bg-[#064E2B] p-5 text-white shadow-[var(--dp-shadow-card)] lg:hidden">
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm font-medium text-[var(--dp-lime-100)]">ចំណូលសរុប</p>
          <span className="grid h-9 w-9 place-items-center rounded-xl bg-white/10">
            <DollarSign size={18} />
          </span>
        </div>
        <p className="mt-1 text-4xl font-extrabold tracking-tight">{totalRevenue}</p>
        <p className="mt-1 text-[11px] text-[var(--dp-lime-100)]">{revenueSubtitle}</p>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <div className="flex items-center gap-3 rounded-xl bg-white/10 px-4 py-3">
            ...
            <div>
              <p className="text-[11px] text-[var(--dp-lime-100)]">ការកម្មង់សរុប</p>
          ...
          <div className="flex items-center gap-3 rounded-xl bg-white/10 px-4 py-3">
            ...
            <div>
              <p className="text-[11px] text-[var(--dp-lime-100)]">ការកម្មង់ជាមធ្យម</p>
